...
// Aggiungi questa funzione dopo tutte le altre funzioni
function setupRelatedProducts() {
    // Delegazione eventi per i pulsanti dei prodotti correlati
    $(document).on('click', '.related-header', function(e) {
        toggleRelatedProducts(this);
    });

    // Delegazione eventi per i pulsanti "Aggiungi" dei prodotti correlati
    $(document).on('click', '.add-related-btn', function(e) {
        e.stopPropagation(); // Impedisce la propagazione all'elemento padre
        const itemName = $(this).closest('.related-item').find('.related-item-name').text();
        const itemPrice = $(this).closest('.related-item').find('.related-item-price').text();

        addToCart({
            id: generateProductId(itemName),
            name: itemName,
            price: parsePrice(itemPrice),
            img: '', // Immagine vuota per i prodotti correlati
            quantity: 1
        });
    });
}

/// Configura i prodotti correlati
function setupRelatedProducts() {
    // Delegazione eventi per i pulsanti dei prodotti correlati
    $(document).on('click', '.related-header', function(e) {
        toggleRelatedProducts(this);
    ...
    // Delegazione eventi per le checkbox dei prodotti correlati
    $(document).on('change', '.related-checkbox', function(e) {
                    ...
                    <div class="cart-item-details">
                        ...
                        <div class="related-products">
                            ...
                            <div class="related-items expanded">
                                ...
                                <!-- Calzini - DISABILITATI per retro -->
                                ...
                                <!-- Pantaloncino - DISABILITATO per retro -->
                                <div class="related-item ${isRetro ? 'disabled' : ''}">
